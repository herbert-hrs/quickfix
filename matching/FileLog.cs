...
namespace QuickFix
{
    ...
    public class FileLog : ILog, System.IDisposable
    {
        ...
        private void Init(string fileLogPath, string prefix)
        {
            ...
            messageLogFileName_ = System.IO.Path.Combine(fileLogPath, prefix + today + ".messages.log");
            eventLogFileName_ = System.IO.Path.Combine(fileLogPath, prefix + today + ".event.log");
        ...
        public void OnIncoming(string msg)
        {
            ...
            lock (sync_)
            {
                ...
                {
                    Message msgLogon = new Message(msg);
                    if(msgLogon.IsSetField(Tags.Password))
                    {
                        msgLogon.SetField(new Password("*********"));

                    }
                    messageLog_.WriteLine(msgLogon.ToString());
                }
                else
                    messageLog_.WriteLine(msg);
            }
        }
