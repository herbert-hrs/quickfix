...
namespace QuickFix
{
    ...
    public class ThreadedSocketReactor
    {
        ...
        public ThreadedSocketReactor(IApplication app,IPEndPoint serverSocketEndPoint, SocketSettings socketSettings)
            : this(app, serverSocketEndPoint, socketSettings, null)
        { }

        public ThreadedSocketReactor(IApplication app, IPEndPoint serverSocketEndPoint, SocketSettings socketSettings,
            QuickFix.Dictionary sessionDict) : this(app, serverSocketEndPoint, socketSettings, sessionDict, null)
        {
            
        }
        internal ThreadedSocketReactor(IApplication app,IPEndPoint serverSocketEndPoint, SocketSettings socketSettings, QuickFix.Dictionary sessionDict, AcceptorSocketDescriptor acceptorDescriptor)
        {
            app_ = app;
        ...
        private void AcceptCallBack(IAsyncResult result)
        {
            ...
            try
            {
                TcpClient client = tcpListener_.EndAcceptTcpClient(result);

                tcpListener_.BeginAcceptTcpClient(AcceptCallBack, null);
           
                if (State.RUNNING == ReactorState)
                {
                    if (socketSettings_.MaxConnections > 0)
                    {
                        lock (sync_)
                        {
                            if (clientThreads_.Count >= socketSettings_.MaxConnections)
                            {
                                client.Close();
                                client.Dispose();
                                return;
                    ...
                    ClientHandlerThread t = new ClientHandlerThread(app_, client, nextClientId_++, sessionDict_, socketSettings_, acceptorDescriptor_);
            ...
            {
                if (State.RUNNING == ReactorState)
                    this.Log("Error connection: " + e.Message);
            }
        }
        ...
        public void Run()
        {
            ...
            {
                this.Log("Error begin accept new client: " + e.Message);
                throw;
            }
